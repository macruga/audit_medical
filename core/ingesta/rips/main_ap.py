...
class  PipeLine():
   ...
   def saveTrace(self):
      with open('trace_ac_3.json', 'w') as file:
   ...
   def saveData(self, data_frame, tipo, start, end):
      if tipo == 'AH':
         try:
            ...
            pd.Timestamp.today()
            # print(data_frame)
            data_frame.to_sql(name='rips_ahintermediamodel_requeridos', con=self.connSIRIUS.engine, if_exists='append', index=False)
            self.trace['file']['success'].append({
               'star': start,
               'end': end,
               'origin': 'save'
      ...
      if tipo == 'AC':
         try:
            ...
            data_frame = data_frame.assign(owner_id = 1)
            data_frame = data_frame.assign(active = 1)            
            data_frame.to_sql(name='rips_acintermediamodel_requeridos', con=self.connSIRIUS.engine, if_exists='append', index=False)
            self.trace['file']['success'].append({
               'star': start,
               'end': end,
               'origin': 'save'
      ...
      if tipo == 'AM':
         try:
            ...
            # data_frame['fecha_salida_usuario'] = data_frame['fecha_salida_usuario'].apply(lambda x: x.strftime('%Y-%m-%d'))            
            data_frame.to_sql(name='rips_amintermediamodel', con=self.connSIRIUS.engine, if_exists='append', index=False)
            self.trace['file']['success'].append({
               'star': start,
               'end': end,
               'origin': 'save'
...
while start < total:
   ...
   query = "SELECT * FROM {} WHERE numero_identificacion in {}".format(TABLES[1], tuple(LISTA_PACIENTES[start:end])) 
   
   data = instance.ingestar(query, tipo, start, end)
   # print(data)
   log.info('Se carga la data en tabla AC intermedia')
