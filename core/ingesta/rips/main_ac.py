...
FILE = 'PacientesMedimas22042021.csv'
...
dfPacientes = pd.read_csv(op.join(settings.DATA_DIR, FILE))
   ...
   def saveData(self, data_frame, tipo, start, end):
      ...
      if tipo == 'AP':
         try:
            ...
            data_frame = data_frame.assign(active = 1)             
            # data_frame['fecha_salida_usuario'] = data_frame['fecha_salida_usuario'].apply(lambda x: x.strftime('%Y-%m-%d'))            
            data_frame.to_sql(name='rips_apintermediamodel', con=self.connSIRIUS.engine, if_exists='append', index=False)
            self.trace['file']['success'].append({
               'star': start,
               'end': end,
               'origin': 'save'
