...
class  PipeLine():
   ...
   def saveTrace(self):
      with open('trace_au_1.json', 'w') as file:
...
while start < total:
   ...
   query = "SELECT * FROM {} WHERE numero_identificacion in {}".format(TABLES[2], tuple(LISTA_PACIENTES[start:end]))    
   data = instance.ingestar(query, tipo, start, end)
   # print(data)
   log.info('Se carga la data en tabla AU intermedia')
