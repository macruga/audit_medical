...
class interconsultaModel(models.Model):
    """
    Modelo de la relación (objeto) interconsulta para las remisiones a interrcionsulta desde una atención
    
    Attributes
    ----------
    description: str
        Descripción de la interconsulta
    
    """
    description = models.CharField(
        primary_key=True,
        max_length=120,
        help_text='Descripción de la interconsulta',
        unique=True,
        null=False,
        validators= [
            noSpacesStartEnd,
    ...
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='user_create_interconsulta', on_delete=models.PROTECT)
