...
from infecciones.serializers.laboratoriosiaas import LabsIaasSerializer


class LabsIaasView(viewsets.ModelViewSet):
    ...
    queryset = LaboratoriosIaasModel.objects.all()
    serializer_class = LabsIaasSerializer
    ...
    def list(self, request): 
        ...
        query = LaboratoriosIaasModel.objects.filter(active = 1).all()
        serializer = LabsIaasSerializer(query, many=True)
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer = LabsIaasSerializer(data=request.data)
        try:
    ...
    def retrieve(self, request, pk=None): 
        query = LaboratoriosIaasModel.objects.filter(id = pk).all()
        serializer = LabsIaasSerializer(query, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    # Peticion PUT
    def update(self, request, pk=None): # pk = id de la aseguradora
        id_ = LaboratoriosIaasModel.objects.get(id=pk)
        serializer = LabsIaasSerializer(instance=id_, data=request.data)
        if serializer.is_valid():
            serializer.save()
    ...
    def destroy(self, request, pk=None):
        ...
        LaboratoriosIaasModel.objects.filter(id = pk ).delete()
